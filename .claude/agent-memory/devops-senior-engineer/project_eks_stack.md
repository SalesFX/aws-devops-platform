...
**How to apply:** When building stacks that need the EKS cluster (e.g., application deployments, ALB ingress), reference `eks/terraform.tfstate` in S3 bucket `devops-ia-production-terraform-state`. Available outputs: `eks_cluster_name`, `eks_cluster_endpoint`, `eks_cluster_certificate_authority_data`, `eks_cluster_security_group_id`, `eks_node_iam_role_arn`.
