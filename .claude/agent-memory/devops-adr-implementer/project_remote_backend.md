...
- Bucket: `devops-ia-production-terraform-state` in `us-east-1`
...
**How to apply:** When implementing consuming stacks, add a backend block to their versions.tf and run `terraform init -migrate-state`. Outputs: `s3_bucket_id`, `s3_bucket_arn`, `s3_bucket_region`.

## MCP validation finding (important)
`bucket_key_enabled` in `aws_s3_bucket_server_side_encryption_configuration` is a sibling of `apply_server_side_encryption_by_default` inside the `rule` block — NOT nested inside `apply_server_side_encryption_by_default`. Provider schema (doc 12311377) is authoritative over ADR guideline text.
